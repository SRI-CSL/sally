...
/** Remove array quantifiers, array lambda terms, and array equalities. **/
class quantifier_free_visitor {
public:
  ...
  quantifier_free_visitor(term_manager& tm, term_to_term_map &subst_map);
  ...
  /** Visit the term and remove quantifiers (children already
      quantifier-free) */
...
void quantifier_free_visitor::error(term_ref t_ref, std::string message) const {
  std::stringstream ss;
...
/* 
   ...
   imap = { i -> (1,4), j -> (1,4) } 
*/
void quantifier_free_visitor::process_array_quantifier(term_ref ref, var_to_interval_map &imap) {
...
/*
     ...
     return value = 0
 */
term_ref quantifier_free_visitor::process_array_lambda(term_ref ref, var_to_interval_map &imap) {
...
void quantifier_free_visitor::get_array_type_bounds(term_ref ref, std::vector<interval_t> &bounds) {
  const term& t = d_tm.term_of(ref);
  term_op op = t.op();
...
quantifier_free_visitor::quantifier_free_visitor(term_manager& tm, term_to_term_map &subst_map)
: d_tm(tm),
...
/** 
    The visitor is building the new quantifier-free term in
    d_subst_map.  The visitor traverses the term in a bottom-up
    fashion and when necessary uses the mapped term (QF terms) rather
    than the original term. This ensures that if at anytime the
    subterm at hand is a quantifier then its body cannot have
    quantifiers. This allows to remove safely all quantifiers even if
    the term is not in prenex normal form.
**/
void quantifier_free_visitor::visit(term_ref t_ref) {
  ...
  case TERM_EQ: {
    ...
    if (d_tm.term_of(t0).op() == VARIABLE && d_tm.term_of(t1).op() == VARIABLE &&
	d_tm.is_array_type(d_tm.type_of(t0)))  {
      /*
	...
	given (= |OM_state_type::next.v| |OM_state_type::state.v|) 
      ...
      */
      ...
      get_array_type_bounds(d_tm.type_of(t0), bounds);
	...
	term_ref read_t0_ref, read_t1_ref;
	rational idx_rat (inst[0].get_signed(), 1) ;
	term_ref idx_term = d_tm.mk_rational_constant(idx_rat);
	read_t0_ref = d_tm.mk_array_read(t0 , idx_term);
	read_t1_ref = d_tm.mk_array_read(t1 , idx_term);	
	for (unsigned idx=1; idx < inst.size(); ++idx) {
	  idx_rat = rational(inst[idx].get_signed(), 1) ;
	  idx_term = d_tm.mk_rational_constant(idx_rat);
	  // -- create t0[i1][i2]...[ik] 
	  read_t0_ref = d_tm.mk_array_read(read_t0_ref , idx_term);
	  // -- create t1[i1][i2]...[ik]	  
	  read_t1_ref = d_tm.mk_array_read(read_t1_ref , idx_term);
	}
	// -- create equality
	flatten_equalities.push_back(d_tm.mk_term(TERM_EQ, read_t0_ref, read_t1_ref));
      }
      d_subst_map[t_ref] = d_tm.mk_and(flatten_equalities);
      return;
    }
    /** Equality between array lambda and array variable **/
    else if (d_tm.term_of(t0).op() == TERM_ARRAY_LAMBDA || d_tm.term_of(t1).op() == TERM_ARRAY_LAMBDA) {
      /*
	...
	a_lambda_t = t0;
	var_t = t1;
      } else if (d_tm.term_of(t1).op() == TERM_ARRAY_LAMBDA) {
	a_lambda_t = t1;
	var_t = t0;
      }
      assert(!a_lambda_t.is_null() && !var_t.is_null());
      ...
      // qf_innermost_body is a quantifier-free formula
      term_ref qf_innermost_body = d_tm.substitute(innermost_body, d_subst_map);          
      //term_ref qf_innermost_body = (!d_subst_map[innermost_body].is_null() ? d_subst_map[innermost_body]: innermost_body);
	...
	for (iit = instantiations.begin(), iet = instantiations.end(); iit!=iet; ++iit){
	  ...
	  // replace quantified indexes in the innermost_body
	  ...
	  flatten_equalities.push_back(d_tm.mk_term(TERM_EQ, d_tm.substitute(qf_innermost_body, q_subst_map), read_ref));
	}
	d_subst_map[t_ref] = d_tm.mk_and(flatten_equalities);
	return;
      }
...
static term_ref rewrite(term_manager& tm, term_ref t)  {
  ...
  typedef quantifier_free_visitor visitor_t;
...
remove_quantifiers::remove_quantifiers(system::context *ctx, std::string id)
: d_ctx(ctx), d_id(id) {}
  
void remove_quantifiers::apply(const system::transition_system *ts) {
...
void remove_quantifiers::apply(const system::state_formula *sf) {
  term_manager &tm = d_ctx->tm();
