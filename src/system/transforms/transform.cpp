...
transform::transform(context* ctx, const transition_system* original)
: d_ctx(ctx)
, d_original(original)
, d_transformed(0)
{
...
factory::register_transform<promote_params_to_state> promote_params_to_state::s_register("promote_params_to_state", 5);
...
transform* factory::mk_transform(std::string id, context* ctx, const transition_system* original) {
  ...
  if (find == s_info.get()->end()) {
    ...
    return find->second.constructor->mk_new(ctx, original);
  }
}
