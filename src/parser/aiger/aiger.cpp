...
class aiger_parser : public internal_parser_interface {
  ...
  void set_cache(const aiger_symbol* t_aiger, expr::term_ref t);

  /** Convert aiger to term while updating the cache */
  expr::term_ref aiger_to_term(const aiger_symbol* t_aiger);
...
void aiger_parser::set_cache(const aiger_symbol* t_aiger, expr::term_ref t) {
  assert(d_aiger_to_term_map.find(t_aiger->lit) == d_aiger_to_term_map.end());
  ...
  // aiger -> term
  d_aiger_to_term_map[t_aiger->lit] = t;
  d_aiger_to_term_map[aiger_not(t_aiger->lit)] = t_not;

  // term -> aiger
  d_term_to_aiger_map[t] = t_aiger->lit;
  d_term_to_aiger_map[t_not] = aiger_not(t_aiger->lit);
}

expr::term_ref aiger_parser::aiger_to_term(const aiger_symbol* t_aiger) {
...
, d_command(0)
{
  ...
  // Add to cache
  for (size_t i = 0; i < a->num_inputs; ++ i) {
    set_cache(a->inputs + i, input_vars[i]);
  }
  for (size_t i = 0; i < a->num_latches; ++ i) {
    set_cache(a->latches + i, state_vars[i]);
  }

  // Get the properties
  for (size_t i = 0; i < a->num_outputs; ++ i) {
    expr::term_ref p_i = aiger_to_term(a->outputs + i);
