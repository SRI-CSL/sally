...
let rec sal_to_expr = function
  ...
  | Sal_ast.Forall (sds, e) ->
      let decls =
        List.flatten (List.map (fun x -> List.map (fun y -> sal_to_decl y (snd x)) (fst x)) sds) in
      let rec make_local = function
        | []    -> sal_to_expr e
        | [d]   -> Local (d, sal_to_expr e)
        | d::ds -> Local (d, make_local ds) in
      make_local decls
  | Sal_ast.True         -> True
  | Sal_ast.False        -> False
  | _                    -> raise (Unexpected_expr "in sal_to_expr")

and

sal_to_decl str = function
...
let rec convert_sal_guardeds next_state res = function
  ...
  | _ -> raise (Unexpected_expr "Unexpected guarded assignment");;
