...
type 'a abs =
  | Id of Var.t
  | Num of Coeff.t
  | Abs of 'a Abstract1.t;;

(* convert from abs type to Texpr1.expr type *)
let abs_to_texpr_expr v = function
  | Id var -> Texpr1.Var var
  | Num cst -> Texpr1.Cst cst
  | _ -> Texpr1.Var v;;

let rec arith_to_texpr man env v is_int binop e1 e2 =
  let v1 = Var.of_string ((Var.to_string v)^"_l") in
  let v2 = Var.of_string ((Var.to_string v)^"_r") in
  (* Solve branches recursively *)
  let abs_e1 = from_expr man env v1 e1 in
  let abs_e2 = from_expr man env v2 e2 in
  let abs =
    match (abs_e1, abs_e2) with
    | (Abs abs1, Abs abs2) -> Abstract1.unify man abs1 abs2
    | (Abs abs1, _) -> abs1
    | (_, Abs abs2) -> abs2
    | (_, _) -> Abstract1.top man env in
  let env' =
    if is_int
  ...
  let texpr =
    let te1 = abs_to_texpr_expr v1 abs_e1 in
    let te2 = abs_to_texpr_expr v2 abs_e2 in
...
from_expr man env v = function
  ...
  | Int i -> Num (Coeff.Scalar (Scalar.of_int i))
  | Float f -> Num (Coeff.Scalar (Scalar.of_float f))
  | Ident e -> Id (Var.of_string (e))
  | Add (e1, e2) -> arith_from_expr man env v false Texpr1.Add e1 e2
  | Sub (e1, e2) -> arith_from_expr man env v false Texpr1.Sub e1 e2
  | Mul (e1, e2) -> arith_from_expr man env v false Texpr1.Mul e1 e2
  | Div (e1, e2) -> arith_from_expr man env v false Texpr1.Div e1 e2
  | Ge (e1, e2) -> comp_from_expr man env v Tcons1.SUPEQ e1 e2
  | Gt (e1, e2) -> comp_from_expr man env v Tcons1.SUP e1 e2
...
let simple_to_abs man p =
  ...
  let invs = invs' in
  let exprs = List.map (from_expr man env (Var.of_string "tmp")) p.exprs in
  List.hd exprs;;
